  ...
  async execute(client, interaction) {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      let _command = interaction.customId.split("_")[1];
        ...
        if(_command == "post") {
          try {
            await client.channels.cache.get(interaction.channel.id).messages.fetch(interaction.message.id).then((message) => { 
              ...
              message.edit({
                components: [new ActionRowBuilder().addComponents(_channel), new ActionRowBuilder().addComponents(_cancel)]
              });
        ...
        try {
          await client.channels.cache.get(interaction.message.channelId).messages.fetch(interaction.message.id).then(message => message.delete());
        } 
        catch(err) {
          console.log(`Error: ${err}`);
        }
      ...
      if(_command == "role") {
        ...
        modal.addComponents(new ActionRowBuilder().addComponents(emojiInput), new ActionRowBuilder().addComponents(descriptionInput));
      ...
      if(_command == "channel") {
        ...
        for(let _id in interaction.message.embeds[0].fields) {
          ...
          _components.addComponents(new ButtonBuilder()
        ...
        interaction.message.embeds[0].fields.unshift({ name: "", value: "\u200b", inline: false });
        interaction.message.embeds[0].fields.push({ name: "", value: "\u200b", inline: false });
      ...
      if(_command == "add") {
        ...
        try {
          await client.channels.cache.get(interaction.channel.id).messages.fetch(_message).then((message) => { 
            let _embed = EmbedBuilder.from(message.embeds[0]).data;
            if(!_embed.fields) { _embed.fields = []; }
            _embed.fields.push({ name: "", value: `${_emoji}: ${roleMention(_role)}` + (_description ? `\n${_description}` : "") + "\n", inline: false });
