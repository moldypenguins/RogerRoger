...
const commandStockpile: DiscordCommand = {
  ...
  async execute(client: DiscordBot, interaction: Interaction): Promise<void> {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      let _town: FoxholeTownData | null = await FoxholeTown.findOne({ id: _location })
      if (!_town) return
      ...
      let _filtered = _choices.filter((c) => c.name.toLowerCase().startsWith(_value.toLowerCase())).slice(0, 25)

      await interaction.respond(_filtered.map((town) => ({ name: town.name, value: town.name })))
      ...
      if (_command == "refresh") {
        ...
        if (await FoxholeStockpile.exists({ id: _sid })) {
          ...
          await FoxholeStockpile.updateOne({ id: _sid }, { $set: { updatedBy: _user._id } })
          _refresh.setDate(_refresh.getDate() + 2)

          let _stockpile = await FoxholeStockpile.findOne({ id: _sid })
          ...
          let building: string = "🏭"
          let town: FoxholeTownData | null = await FoxholeTown.findById(_stockpile.town)
          if (!town) return
        ...
        if (_action == "yes") {
          if (_guild.stockpilesChannelId && (await FoxholeStockpile.exists({ id: _sid }))) {
            //console.log(`SID: ${util.inspect(interaction.message.id, true, 1, true)}`);
            let _stockpile: FoxholeStockpileData | null = await FoxholeStockpile.findOne({ id: _sid })
            if (_stockpile && _stockpile.messageId) {
              ...
              if (_stockchan?.isTextBased() && "send" in _stockchan) {
                ...
                _stockchan.messages.fetch(_stockpile.messageId).then(async (message: Message) => {
                  message.delete()
                  await FoxholeStockpile.deleteOne({ id: _sid })
                })
              }
            }
          }
